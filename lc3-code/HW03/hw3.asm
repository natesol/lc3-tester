...
BestStudent_Save_R0 .FILL #0
BestStudent_Save_R1 .FILL #0
BestStudent_Save_R2 .FILL #0
...
BestStudent:
	; - Store Registers For Backup ---
	ST R0, BestStudent_Save_R0
	ST R1, BestStudent_Save_R1
	ST R2, BestStudent_Save_R2
	...
	AND R3, R3, #0								; R3: counter to itarate array
	AND R4, R4, #0								; R4: General purpose (for calculations, load and store).
	AND R5, R5, #0								; R5: ???
	AND R6, R6, #0								; R6: ???
	AND R7, R7, #0								; R7: ???
	...
	; reinitialize list
	End_BestStudent:
		ADD R0, R7, #0								; R0: list pointer.
		AND R1, R1, #0								; R1: #3 - lists counter
		AND R2, R2, #0								; R2: #10 - elemnets counter
		AND R3, R3, #0								; R3: #0 - 0 vector for init
		AND R4, R4, #0								; R4: general purpose
		;
		ADD R1, R1, #3
		;
		Reinitialize_Loop:
			...
			Init_Element_Loop:
				...
				Continue_Init:
					ADD R0, R0, #7
					ADD R2, R2, #-1
			BRp Init_Element_Loop
			;
			ADD R0, R0, #1
			ADD R1, R1, #-1
		BRp Reinitialize_Loop
	;
	; - Reload Registers Original Values ---
	LD R0, BestStudent_Save_R0
	LD R1, BestStudent_Save_R1
	LD R2, BestStudent_Save_R2
